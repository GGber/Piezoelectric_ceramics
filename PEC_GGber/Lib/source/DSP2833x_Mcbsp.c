...
// McBSP-B register settings
    ...
    McbspbRegs.SPCR1.bit.CLKSTP = 3;    // Clock stop mode, without clock delay.

	McbspbRegs.MFFINT.all=0x0;			// Disable all interrupts

	McbspbRegs.RCR1.bit.RFRLEN1 = 0;    // Receive frame length 1 = 1.
    McbspbRegs.RCR1.bit.RWDLEN1 = 0x2;  // Receive word length:16-bit word.
    McbspbRegs.RCR2.bit.RPHASE = 0;     // Single-phase frame.
    McbspbRegs.RCR2.bit.RDATDLY = 0x01; // FSX setup time 1 in master mode. 0 for slave mode (Receive).

    McbspbRegs.XCR1.bit.XFRLEN1 = 0;    // Transmit frame length 1 = 1.
    McbspbRegs.XCR1.bit.XWDLEN1 = 0x2;  // Transmit word length 1:16-bit word
    McbspbRegs.XCR2.bit.XPHASE = 0;     // Single-phase frame.
    McbspbRegs.XCR2.bit.XDATDLY = 0x01; // FSX setup time 1 in master mode. 0 for slave mode (Transmit).

    McbspbRegs.SRGR1.bit.FWID = 0;      // Frame Width = 1 CLKG period
    McbspbRegs.SRGR1.bit.CLKGDV = 0;    // CLKG frequency = (Input clock frequency)/ (CLKGDV + 1)
    McbspbRegs.SRGR2.bit.CLKSM = 1;     // SCLKME=0, CLKSM=1, Input Clock For Sample Rate Generator:LSPCLK
    McbspbRegs.SRGR2.bit.FPER = 0;

    McbspbRegs.PCR.bit.FSXM = 1;        // Transmit frame synchronization is supplied by the sample rate generator.
    McbspbRegs.PCR.bit.FSRM = 1;        // Receive frame synchronization is supplied by the sample rate generator.
    McbspbRegs.PCR.bit.CLKXM = 1;       // Sample rate generator drives the internal transmit clock (CLKX).
    McbspbRegs.PCR.bit.CLKRM = 1;       // Internal MCLKR is driven by the sample rate generator.
    McbspbRegs.PCR.bit.SCLKME = 0;      // SCLKME=0, CLKSM=1, Input Clock For Sample Rate Generator:LSPCLK
    McbspbRegs.PCR.bit.FSXP = 1;        // Transmit frame-synchronization pulses are active low.
    McbspbRegs.PCR.bit.FSRP = 1;        // Receive frame-synchronization pulses are active low.
    McbspbRegs.PCR.bit.CLKXP = 0;       // Transmit data is sampled on the rising edge of CLKX.
    McbspbRegs.PCR.bit.CLKRP = 0;       // Receive data is sampled on the rising edge of MCLKR.
    delay_loop();                // Wait at least 2 SRG clock cycles
    McbspbRegs.MFFINT.bit.XINT = 0;     // Disable Transmit Interrupts
    McbspbRegs.MFFINT.bit.RINT = 0;     // Enable Receive Interrupts
